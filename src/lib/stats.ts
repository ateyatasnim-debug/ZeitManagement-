...
// All focus attempts, completed or interrupted — used for every *time*
// total, since interrupted time was still real focus time and should count.
function allFocus(sessions: FocusSession[]): FocusSession[] {
  return sessions.filter((s) => s.type === 'focus')
}

// Only fully completed pomodoros — used for "discipline" metrics like
// average session length and success rate.
...
export function computeSummary(sessions: FocusSession[]): SummaryStats {
  ...
  const minutesSince = (start: number) =>
    all.filter((s) => s.startedAt >= start).reduce((sum, s) => sum + s.actualSeconds / 60, 0)

  const focusMinutesToday = all
    .filter((s) => toDateKey(s.startedAt) === today)
    .reduce((sum, s) => sum + s.actualSeconds / 60, 0)

  const totalMinutesAll = all.reduce((sum, s) => sum + s.actualSeconds / 60, 0)
  const totalMinutesCompleted = completed.reduce((sum, s) => sum + s.actualSeconds / 60, 0)
  const avgSessionMinutes = completed.length > 0 ? totalMinutesCompleted / completed.length : 0
  const successRate = all.length > 0 ? completed.length / all.length : 0

  const hourTotals = new Array(24).fill(0)
  for (const s of all) hourTotals[new Date(s.startedAt).getHours()] += s.actualSeconds / 60
  const bestHour = hourTotals.reduce((best, v, i) => (v > hourTotals[best] ? i : best), 0)
  const bestHourLabel = totalMinutesAll > 0 ? `${String(bestHour).padStart(2, '0')}–${String((bestHour + 1) % 24).padStart(2, '0')} Uhr` : null

  const longestSessionMinutes = all.reduce((max, s) => Math.max(max, s.actualSeconds / 60), 0)
...
export function focusPerHour(sessions: FocusSession[]): HourPoint[] {
  ...
  for (const s of all) totals[new Date(s.startedAt).getHours()] += s.actualSeconds / 60
...
export function focusPerProject(sessions: FocusSession[], projects: Project[]): ProjectPoint[] {
  const all = allFocus(sessions).filter((s) => s.projectId)
...
export function weekComparison(sessions: FocusSession[]): WeekCompare[] {
  ...
  return days.map((label, i) => {
    ...
    const thisWeek = all.filter((s) => toDateKey(s.startedAt) === thisKey).reduce((sum, s) => sum + s.actualSeconds / 60, 0)
    const lastWeek = all.filter((s) => toDateKey(s.startedAt) === lastKey).reduce((sum, s) => sum + s.actualSeconds / 60, 0)
